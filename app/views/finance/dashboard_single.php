...
<div class="container-fluid p-0">
    <!-- Header with Breadcrumbs & Action Hub -->
            ...
            <div>
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-1">
                        ...
                        <li class="breadcrumb-item active"><?= htmlspecialchars($churchName) ?></li>
                    </ol>
                </nav>
                <h4 class="mb-0 fw-bold text-dark"><i class="bx bx-building-house text-primary me-1"></i> <?= htmlspecialchars($churchName) ?> - Financial Management</h4>
            </div>
            <div class="d-flex gap-2 flex-wrap">
                <a href="<?= AssetHelper::url("churches/{$churchId}/budgets") ?>" class="btn btn-outline-primary">
                    <i class="bx bx-pie-chart-alt-2 me-1"></i> Budgets
                </a>
                <a href="<?= AssetHelper::url("churches/{$churchId}/pledges") ?>" class="btn btn-outline-success">
                    <i class="bx bx-gift me-1"></i> Pledges
                </a>
                <a href="<?= AssetHelper::url("churches/{$churchId}/finance/cashflow") ?>" class="btn btn-outline-info">
                    <i class="bx bx-line-chart me-1"></i> Cashflow & YoY
                </a>
                <a href="<?= AssetHelper::url("churches/{$churchId}/finance/audit-trail") ?>" class="btn btn-outline-secondary">
                    <i class="bx bx-shield-quarter me-1"></i> Audit Trail
                </a>
                <a href="<?= AssetHelper::url("churches/{$churchId}/finance/create") ?>" class="btn btn-primary">
    ...
    <div class="p-4">
                ...
                <div class="card border-0 shadow-sm rounded-3 h-100 bg-gradient" style="background: linear-gradient(135deg, #11998e 0%, #38ef7d 100%); color: white;">
                    <div class="card-body p-3">
                        <div class="d-flex align-items-center justify-content-between mb-2">
                            <span class="text-white-50 small text-uppercase fw-semibold">Total Income</span>
                        ...
                        <small class="text-white-50"><i class="bx bx-check-circle me-1"></i> <?= $summary['record_count'] ?> Transactions Logged</small>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-4">
                <div class="card border-0 shadow-sm rounded-3 h-100 bg-gradient" style="background: linear-gradient(135deg, #eb3349 0%, #f45c43 100%); color: white;">
                    <div class="card-body p-3">
                        ...
                        <small class="text-white-50">Local church disbursements</small>
                    </div>
                </div>
            </div>
        ...
        <div class="card border-0 shadow-sm rounded-3 mb-4">
            <div class="card-header bg-transparent py-3 border-bottom d-flex align-items-center justify-content-between">
                <h5 class="mb-0 fw-bold text-dark"><i class="bx bx-line-chart me-1 text-primary"></i> 6-Month Financial Flow</h5>
                <a href="<?= AssetHelper::url("churches/{$churchId}/finance/cashflow") ?>" class="btn btn-sm btn-link text-primary text-decoration-none">Full Cashflow & YoY &rarr;</a>
            </div>
            <div class="card-body">
                <div id="singleChurchFinanceChart" style="min-height: 280px;"></div>
            </div>
        </div>
            ...
            <div class="card-body p-3">
                <form method="GET" class="row g-3 align-items-end">
                    ...
                    <div class="col-md-4 d-flex gap-2">
                        ...
                        <a href="<?= AssetHelper::url("churches/{$churchId}/finance") ?>" class="btn btn-light" title="Reset"><i class="bx bx-refresh"></i></a>
                        <a href="<?= AssetHelper::url("churches/{$churchId}/finance/export/csv") ?>" class="btn btn-outline-secondary" title="Export CSV"><i class="bx bx-download"></i></a>
                    </div>
                </form>
            </div>
        </div>
        ...
        <div class="card border-0 shadow-sm rounded-3">
            <div class="card-header bg-transparent py-3 border-bottom d-flex align-items-center justify-content-between">
                <h5 class="mb-0 fw-bold text-dark"><i class="bx bx-receipt me-1 text-primary"></i> Financial Transactions (<?= count($records) ?>)</h5>
                <a href="<?= AssetHelper::url("churches/{$churchId}/finance/create") ?>" class="btn btn-sm btn-primary">
            ...
            <div class="card-body p-0">
                <?php if (empty($records)): ?>
                    <div class="text-center py-5">
                        ...
                        <h6 class="text-muted">No financial records found.</h6>
                    </div>
                <?php else: ?>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <?php if (!empty($r['payment_method'])): ?>
                                                <div class="small text-muted"><i class="bx bx-credit-card me-1"></i><?= ucfirst(str_replace('_', ' ', $r['payment_method'])) ?></div>
                                            <?php endif; ?>
                                        </td>
                                        ...
                                        <td>
                                            <span class="badge bg-soft-primary text-primary">
                                                <?= htmlspecialchars($r['unit_name'] ?? 'Church-wide') ?>
                                            </span>
                                        </td>
                                        <td class="text-end pe-4">
                                            <a href="<?= AssetHelper::url("churches/{$churchId}/finance/{$r['id']}") ?>" class="btn btn-sm btn-light">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    var chart = new ApexCharts(document.querySelector("#singleChurchFinanceChart"), options);
    chart.render();
});
</script>
